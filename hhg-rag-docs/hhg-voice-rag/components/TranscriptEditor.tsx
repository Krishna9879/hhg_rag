  ...
  return (
    <div className="w-full flex flex-col gap-2">
      <div className="relative flex items-center w-full rounded-2xl bg-[#141A18]/90 border border-white/10 shadow-xl focus-within:border-emerald-500/60 focus-within:ring-4 focus-within:ring-emerald-500/15 transition-all">
        {/* Left Search Icon */}
        <div className="pl-3 sm:pl-5 text-[#9AA6A2]/70 flex items-center pointer-events-none shrink-0">
        ...
        <input
          ...
          disabled={isLoading}
          className="w-full px-2.5 sm:px-4 py-3 sm:py-4 bg-transparent text-[#F5F7F6] text-sm sm:text-base placeholder-[#9AA6A2]/50 focus:outline-none font-sans min-w-0"
        />

        <div className="pr-2 sm:pr-3 flex items-center gap-1.5 sm:gap-2 shrink-0">
          {/* Clear Button */}
          {query.trim() && !isLoading && (
            <button
              type="button"
              onClick={() => onChange("")}
              className="p-1 sm:p-1.5 rounded-lg text-[#9AA6A2] hover:text-white hover:bg-white/10 transition-colors"
              title="Clear input"
            >
